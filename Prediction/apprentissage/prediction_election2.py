...
# Filtrer et trier les données par année et par Ratio_voix_exprime_adjusted
df_sorted = df.sort_values(['Annee', 'Ratio_voix_exprime_adjusted'], ascending=[True, False])

# Créer une fonction pour récupérer et dupliquer les deux premières lignes de chaque année
def extraire_et_dupliquer(group):
    # Sélectionner les deux premières lignes
    top_2 = group.head(2).copy()
    # Dupliquer les lignes
    top_2['Tour'] = 2
    # Retourner les deux lignes dupliquées
    return top_2

# Appliquer la fonction à chaque groupe d'année
df_duplique = df_sorted.groupby('Annee', group_keys=False).apply(extraire_et_dupliquer)

# On retire les colonnes Pourcentage_Votants,Ratio_voix_exprime,Ratio_voix_exprime_adjusted
...
df_pred.to_csv('./Prediction/data/predictions_2025_2027_top_2_with_predictions.csv', index=False)
...
df_pred = pd.read_csv('./Prediction/data/predictions_2025_2027_top_2_with_predictions.csv')
